...
public class MainActivity extends Activity {
 ...
 Button tacticCycle(String label,String value,String[] options,int kind){Button b=bt(label+" · "+value);b.setOnClickListener(v->{String nv=cycle(kind==0?pressLine:kind==1?pressIntensity:kind==2?pressTrap:lossReaction,options);if(kind==0)pressLine=nv;else if(kind==1)pressIntensity=nv;else if(kind==2)pressTrap=nv;else lossReaction=nv;getSharedPreferences("dumok_tactic",0).edit().putString("line",pressLine).putString("intensity",pressIntensity).putString("trap",pressTrap).putString("loss",lossReaction).apply();tactic();});return b;}
 void loadPressPrefs(){android.content.SharedPreferences sp=getSharedPreferences("dumok_tactic",0);pressLine=sp.getString("line",pressLine);pressIntensity=sp.getString("intensity",pressIntensity);pressTrap=sp.getString("trap",pressTrap);lossReaction=sp.getString("loss",lossReaction);}
 void tactic(){
  ...
  pc.addView(tacticCycle("공을 잃은 뒤",lossReaction,new String[]{"즉시 압박","재정비"},3));body.addView(pc);
 ...
 void match(Fixture f){
  ...
  loadPressPrefs();SmoothPitch pitch=new SmoothPitch(this, tacticalCoords(), matchProfiles(), matchRoles(), matchInst(), matchSlots(),pressLine,pressIntensity,pressTrap,lossReaction);
  ...
  final boolean homeIsBlue=f.h.name.equals(club);final boolean[] paused={false};final int[] subsUsed={0},subWindows={0};final boolean[] windowOpen={false};
 ...
 static class SmoothPitch extends View{
  ...
  Player[] simPlayer=new Player[22]; String[] simRole=new String[22],simInst=new String[22],simSlot=new String[22]; String pressLine,pressIntensity,pressTrap,lossReaction; int primaryPress=-1,coverPress=-1,blockPress=-1,pressTicks=0; boolean pressActive=false;
  float[][] xy=new float[22][2], base=new float[22][2], dest=new float[22][2], vel=new float[22][2];
   ...
   int[] roleState=new int[22],yellowCards=new int[22]; boolean[] sentOff=new boolean[22]; int animFrame=0;float[] matchRating=new float[22];int lastPasser=-1;float lastActionThreat=0f;
  int restartType=0,restartTaker=-1;float restartX=.5f,restartY=.5f; // 1 indirect FK, 2 direct FK, 3 penalty
  float[][] principleTarget=new float[22][2];float[] perceptionLag=new float[22],riskMemory=new float[22];int transitionTicks=0;boolean previousBlue=true;
  float bx=.5f,by=.5f,btx=.5f,bty=.5f; int owner=0,receiver=-1,state=0,ticks=0,dribbleDef=-1,dribblePhase=0;
  float lastBallX=.5f,lastBallY=.5f;int ballIdleFrames=0;
  ...
  Handler h=new Handler(Looper.getMainLooper());
  SmoothPitch(Context c,float[][] userShape,Player[] userPlayers,String[] userRoles,String[] userInst,String[] userSlots,String pl,String pi,String pt,String lr){
   super(c);pressLine=pl;pressIntensity=pi;pressTrap=pt;lossReaction=lr;
   ...
   for(int i=0;i<22;i++){renderXY[i][0]=xy[i][0];renderXY[i][1]=xy[i][1];renderHeadingX[i]=1f;moveBlend[i]=0f;perceptionLag[i]=.72f+r.nextFloat()*.38f;riskMemory[i]=r.nextFloat();}
  ...
  void step(){
   ...
   if(state==1){if(closeBall()){int recv=receiver;float gain=Math.max(-.12f,laneThreat(xy[recv][0],xy[recv][1],recv<11)-lastActionThreat);rate(lastPasser,.015f+Math.max(0f,gain)*.16f);owner=recv;receiver=-1;ballFlying=false;state=0;eventText=stateName(owner)+" 패스 성공";newLog=true;shape();}return;}
   if(state==2){
   ...
   if(state==7){ // visible 1v1: feint -> burst -> defender recovery
    ...
    if(dribblePhase==2){float dx=dest[owner][0]-xy[owner][0],dy=dest[owner][1]-xy[owner][1];if(dx*dx+dy*dy<.0012f){state=0;dribbleDef=-1;dribblePhase=0;rate(owner,.08f+.08f*laneThreat(xy[owner][0],xy[owner][1],owner<11));if(dribbleDef>=0)rate(dribbleDef,-.035f);eventText="돌파 성공 · 전진";newLog=true;shape();}return;}
   }
  ...
  void setupRestart(boolean attackingBlue,float x,float y,int type){restartType=type;restartX=clip(x);restartY=clip(y);restartTaker=nearestActiveTo(restartX,restartY,attackingBlue);owner=restartTaker;receiver=-1;ballFlying=false;state=0;bx=restartX;by=restartY;if(restartTaker>=0){xy[restartTaker][0]=restartX;xy[restartTaker][1]=restartY;dest[restartTaker][0]=restartX;dest[restartTaker][1]=restartY;}shape();}
  void takeRestart(){
   if(restartType==0||restartTaker<0)return;boolean blue=restartTaker<11;int t=restartType;restartType=0;
   if(t==3){owner=restartTaker;bx=blue?.88f:.12f;by=.5f;shoot(blue);eventText="⚽ 페널티킥";newLog=true;return;}
   if(t==2&&laneThreat(bx,by,blue)>.58f&&r.nextFloat()<.58f){owner=restartTaker;shoot(blue);eventText="직접 프리킥 · 슈팅";newLog=true;return;}
  ...
  void tackle(int defender){
   ...
   if(r.nextFloat()<tw){float danger=laneThreat(xy[owner][0],xy[owner][1],owner<11);rate(defender,.10f+.14f*danger);rate(owner,-.05f-.06f*danger);if(pressActive&&primaryPress>=0&&primaryPress!=defender)rate(primaryPress,.035f+.035f*danger);pressActive=false;primaryPress=coverPress=blockPress=-1;owner=defender;receiver=-1;ballFlying=false;state=0;eventText="태클 성공 · 소유권 전환";newLog=true;shape();}
   else if((owner%11)>=8){startDribbleDuel(defender);}
   else{eventText="압박을 버티고 전진";newLog=true;}
  }
  float secondLastDefenderLine(boolean blueAttack){int lo=blueAttack?11:0,hi=lo+11;float first=blueAttack?2f:-1f,second=first;for(int i=lo;i<hi;i++){if(i%11==0)continue;float x=xy[i][0];if(blueAttack){if(x<first){second=first;first=x;}else if(x<second)second=x;}else{if(x>first){second=first;first=x;}else if(x>second)second=x;}}return second;}
  boolean isOffsideTarget(int passer,int target,boolean blue){if(target<0)return false;float tx=xy[target][0],px=xy[passer][0];if(blue){if(tx<=.5f||tx<=px)return false;return tx>secondLastDefenderLine(true)+.006f&&tx>bx+.006f;}else{if(tx>=.5f||tx>=px)return false;return tx<secondLastDefenderLine(false)-.006f&&tx<bx-.006f;}}
  void awardOffside(boolean blue){setupRestart(!blue,bx,by,1);eventText="오프사이드 · 간접 프리킥";newLog=true;}
  void pass(boolean blue){
   lastPasser=owner;lastActionThreat=laneThreat(xy[owner][0],xy[owner][1],blue);int lo=blue?0:11,hi=lo+11,best=-1;float bestScore=-99f,dir=blue?1f:-1f;
   for(int i=lo;i<hi;i++){if(!active(i)||i==owner||isOffsideTarget(owner,i,blue))continue;float dx=xy[i][0]-xy[owner][0],dy=xy[i][1]-xy[owner][1],d=(float)Math.sqrt(dx*dx+dy*dy);if(d>.48f)continue;
    float forward=dir*dx,space=spaceAt(xy[i][0],xy[i][1],blue),threat=laneThreat(xy[i][0],xy[i][1],blue),score=.35f*space+.34f*threat+.20f*Math.max(-.2f,forward)+.11f*(1f-Math.min(1f,d/.48f));
    if(isForwardRole(i)&&transitionTicks>0)score+=.16f;if(i==primaryPress)score-=.08f;score+=(r.nextFloat()-.5f)*(.18f-.10f*passQ(owner));if(score>bestScore){bestScore=score;best=i;}}
   if(best<0)best=nearestMate(owner,xy[owner][0],xy[owner][1]);receiver=best;state=1;lastEventBlue=blue;btx=xy[receiver][0];bty=xy[receiver][1];ballFlying=true;eventText="→ 패스";newLog=true;
  }
  void through(boolean blue){
   lastPasser=owner;lastActionThreat=laneThreat(xy[owner][0],xy[owner][1],blue);int b=blue?0:11,best=-1;float bs=-99f;
   for(int i=b;i<b+11;i++){if(!active(i)||i==owner||!isForwardRole(i)||isOffsideTarget(owner,i,blue))continue;float sc=laneThreat(xy[i][0],xy[i][1],blue)+spaceAt(xy[i][0],xy[i][1],blue);if(sc>bs){bs=sc;best=i;}}
   ...
   for(int j=0;j<22;j++){
    ...
    if(attacking){
     ...
     if(local==0){tx=blue?.075f:.925f;ty=.5f+(ballY-.5f)*.12f;roleState[j]=0;}
      ...
      // Rest defence: when many teammates are ahead of the ball, deeper players hold instead of all joining.
      int ahead=0;for(int k=(j<11?0:11);k<(j<11?11:22);k++){if(k==j)continue;if((blue&&xy[k][0]>ballX)||(!blue&&xy[k][0]<ballX))ahead++;}
      if(ahead>=5&&local>=1&&local<=6&&!forward){float safe=blue?Math.min(tx,ballX-.10f):Math.max(tx,ballX+.10f);tx=clip(safe);roleState[j]=5;}
      // Support triangle: if too close to the carrier, fan away into the more open adjacent lane.
      float db=(float)Math.sqrt((tx-ballX)*(tx-ballX)+(ty-ballY)*(ty-ballY));if(db<.11f&&!forward){float up=spaceAt(tx,clip(ty-.10f),blue),dn=spaceAt(tx,clip(ty+.10f),blue);ty=clip(ty+(up>dn?-.08f:.08f));}
     }
    }else{
  ...
  void updateIndependentMovement(){
   if(animFrame%7!=0)return;boolean blue=owner>=0?owner<11:lastPossessionBlue;float ballX=owner>=0?xy[owner][0]:bx,ballY=owner>=0?xy[owner][1]:by;footballPrinciples(blue,ballX,ballY);
  }
